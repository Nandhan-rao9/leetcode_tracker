...
export default function CompanyView({ activeCompany, problems, onBack }) {
  ...
  // Use activeCompany if available, otherwise use the name from URL
  const companyName = activeCompany?.name || decodeURIComponent(name || "");
  const storageKey = `plan_${companyName}`;
  ...
  const generatePlan = async () => {
    try {
      ...
      const res = await api.post(
        `/companies/${encodeURIComponent(companyName)}/smart_plan`,
        {
          num,
  ...
  if (!companyName) {
    return (
      <div className="text-center text-slate-400 py-10">
        No company selected. <button onClick={() => navigate("/companies")} className="text-cyan-400 underline">Go to Companies</button>
      </div>
    );
  }
  ...
  return (
    <section>
      <button onClick={handleBack} className="text-cyan-400 mb-4 hover:underline">
        ← Back to Companies
      </button>
