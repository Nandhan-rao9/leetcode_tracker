...
// --- Reusable Problem Card Component (Updated) ---
const ProblemCard = ({ problem }) => (
  <div style={cardStyle}>
    ...
    <div>
      {/* --- LOGIC UPDATED --- */}
      {/* Read from the simple 'all_topics' array */}
...
function Revision({ solvedProblems }) {
  const [dailyProblem, setDailyProblem] = useState(null);
  const [similarProblems, setSimilarProblems] = useState([]);
  const [loadingSimilar, setLoadingSimilar] = useState(false);

  // 1. Find the "Daily" Problem (Unchanged)
  useMemo(() => {
  ...
  // 2. Fetch similar problems (Logic Updated)
  useEffect(() => {
    if (dailyProblem) {
      const fetchSimilar = async () => {
        ...
        // --- LOGIC UPDATED ---
        // Get the topics from the 'all_topics' array
        const tags = dailyProblem.all_topics.join(',');
  ...
  // (Rest of the rendering is unchanged)
  return (
    <div>
